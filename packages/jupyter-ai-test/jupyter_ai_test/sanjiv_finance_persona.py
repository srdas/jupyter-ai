...
FINANCIAL_DATASETS_API_KEY = 'fa43dbd3-5850-4f0d-9714-751770b702a7'
...
class SanjivFinancePersona(BasePersona, BaseChatHandler):
    """
    ...
    # Set variables for the environment to use API keys
    env_api_keys_from_config()
    ...
    async def process_message(self, message: Message):
        ...
        if "/ticker" in variables.input:
            ...
            if msg:
                ...
                # prices = sample_prices('/Users/sanjivda/sample_prices.json')
                # Convert the prices to a DataFrame
                df = pd.DataFrame(prices["prices"])
                # self.reply(f"====> Prices: {prices}")
                self.reply(f"====> Close: {df.close.to_list()}")
            else:
                self.reply("Error: No ticker provided. Provide it in the format /prices <ticker>")
        elif "/financials" in variables.input:
            ft = FinTools()
            msg = variables.input.split("/financials", 1)[1].strip()
            ...
            if msg:
                ...
                self.reply(series)
        elif "/forecast" in variables.input:
            print("====> Forecast", variables.input)
            ft = FinTools()
            # fdtools = FinancialDatasetsTools(api_key=FINANCIAL_DATASETS_API_KEY)
            msg = variables.input.split("/forecast", 1)[1].strip()
            ...
            if msg:
                ...
                print(f"====> Ticker: {ticker} {start_date} {end_date} {prediction_length}")
                # series = fdtools.get_stock_prices(ticker=ticker, start_date=start_date, end_date=end_date)
                series = sample_prices('/Users/sanjivda/sample_prices.json')
                df = pd.DataFrame(series["prices"])
                series = df.close.to_list()
                quantiles, _ = ft.forecast_data(series=series, prediction_length=prediction_length)
                self.reply(f"====> Quantiles: {quantiles}")
                # Plot the forecast data
                # ft.plot_forecast_data(series=series, quantiles=quantiles, prediction_length=prediction_length)
            else:
                self.reply("Error: No ticker provided. Provide it in the format /forecast <ticker>")
        else:
    ...
    def agno_ticker(self, message: Message):
        ...
        agent = Agent(
            ...
            tools = [FinancialDatasetsTools(enable_company_info=True, enable_prices=True)],
        ...
        # Print the response  
        self.reply(f"====> ticker response: {response}")
